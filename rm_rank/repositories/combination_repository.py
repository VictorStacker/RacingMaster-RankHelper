...
class CombinationRepository:
            ...
            for position, config in enumerate(vehicle_configs, start=1):
                ...
                if vehicle:
                    combination = CurrentCombination(
                        vehicle_id=vehicle.id, position=position
                    )
                    self.session.add(combination)

            self.session.commit()
            logger.info(f"已保存当前组合，共 {len(vehicle_configs)} 辆车")
    ...
    def get_current_combination(self) -> List[VehicleConfig]:
        ...
        try:
            ...
            logger.info(f"获取当前组合，共 {len(result)} 辆车")
            return result
    ...
    def clear_current_combination(self) -> None:
        ...
        """
        try:
            self.session.query(CurrentCombination).delete()
            self.session.commit()
            logger.info("已清空当前组合")
    ...
    def has_current_combination(self) -> bool:
        ...
        """
        try:
            count = self.session.query(CurrentCombination).count()
            return count > 0
        except Exception as e:
